...
# in such a way that they would change the Ruby RPM
...
if [[ -z "$MOCK_UNBUMPED_RUBY_PACKAGE_VERSIONS" ]]; then
    ...
    IFS=$'\n' UNBUMPED_RUBY_PACKAGE_VERSIONS=($("$SELFDIR"/determine-unbumped-ruby-package-versions.rb "$ROOTDIR/config.yml" config-latest-release.yml))
else
